package net.voidless.voidless.worldgen.portal.shape;
...
public class VoidPortalShape {
        ...
        public static Optional<VoidPortalShape> findEmptyPortalShape(LevelAccessor pLevel, BlockPos pBottomLeft, Direction.Axis pAxis) {
            return findPortalShape(pLevel, pBottomLeft, (p_77727_) -> {
                return p_77727_.isValid() && p_77727_.numPortalBlocks == 0;
            }, pAxis);
        }

        public static Optional<VoidPortalShape> findPortalShape(LevelAccessor pLevel, BlockPos pBottomLeft, Predicate<VoidPortalShape> pPredicate, Direction.Axis pAxis) {
            Optional<VoidPortalShape> optional = Optional.of(new VoidPortalShape(pLevel, pBottomLeft, pAxis)).filter(pPredicate);
            if (optional.isPresent()) {
                return optional;
            } else {
                Direction.Axis direction$axis = pAxis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
                return Optional.of(new VoidPortalShape(pLevel, pBottomLeft, direction$axis)).filter(pPredicate);
            }
        }

        public VoidPortalShape(LevelAccessor pLevel, BlockPos pBottomLeft, Direction.Axis pAxis) {
            this.level = pLevel;
            this.axis = pAxis;
        ...
        public static Vec3 getRelativePosition(BlockUtil.FoundRectangle pFoundRectangle, Direction.Axis pAxis, Vec3 pPos, EntityDimensions pEntityDimensions) {
            ...
            double d5;
            if (d0 > 0.0) {
                d5 = (double)blockpos.get(pAxis) + (double)pEntityDimensions.width() / 2.0;
                d2 = Mth.clamp(Mth.inverseLerp(pPos.get(pAxis) - d5, 0.0, d0), 0.0, 1.0);
            } else {
                d2 = 0.5;
            }
            ...
            if (d1 > 0.0) {
                ...
                d5 = Mth.clamp(Mth.inverseLerp(pPos.get(direction$axis1) - (double)blockpos.get(direction$axis1), 0.0, d1), 0.0, 1.0);
            } else {
                d5 = 0.0;
            }
