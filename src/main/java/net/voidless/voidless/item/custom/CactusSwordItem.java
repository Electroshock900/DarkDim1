...
public class CactusSwordItem extends SwordItem {
    public CactusSwordItem(Tier p_43269_, int p_43270_, float p_43271_, Properties properties) {
    ...
    public void releaseUsing(ItemStack stack, Level level, LivingEntity shooter, int ticksRemaining) {
        if (shooter instanceof Player player) {
            ...
            if (!level.isClientSide) {
                Cactus_Spine arrow = new Cactus_Spine(level, player);
                arrow.tickCount = 35;
                ...
                stack.hurtAndBreak(1, shooter, shooter.getEquipmentSlotForItem(this.getDefaultInstance()));
