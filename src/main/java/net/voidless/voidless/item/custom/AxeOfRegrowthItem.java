...
//                .collect(Collectors.toList());
    ...
    private Item getSaplingByLeaf(Level pLevel,List<BlockPos> leavesPos) {
        if (leavesPos.isEmpty()) {
            ...
            return ForgeRegistries.ITEMS.getValue(ResourceLocation.parse("minecraft:air"));
        }

        BlockState state = pLevel.getBlockState(leavesPos.get(0));
        Block block = state.getBlock();

        String treeType = ForgeRegistries.BLOCKS.getKey(block).toString().replace("_leaves", "");
        String sapling;
        ...
        return ForgeRegistries.ITEMS.getValue(ResourceLocation.parse(sapling));
    }
