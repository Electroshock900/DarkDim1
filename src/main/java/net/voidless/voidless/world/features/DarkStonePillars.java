package net.voidless.voidless.world.features;
...
public class DarkStonePillars extends SimpleBlockFeature {
    public DarkStonePillars(Codec<SimpleBlockConfiguration> pCodec) {
        super(pCodec);
    }
    ...
    public boolean place(FeaturePlaceContext pContext) {
        ...
        level.setBlock(origin.below(), ModBlocks.DARK_STONE.get().defaultBlockState(), 3);
        level.setBlock(origin.above(columnHeight), ModBlocks.DARK_STONE.get().defaultBlockState(), 3);

        for(int i = 1; i < columnHeight + 1; i++) {
            int newY = origin.getY() + i - 1;
            BlockPos pos = new BlockPos(origin.getX(), newY, origin.getZ());
            level.setBlock(pos, ModBlocks.DARK_STONE.get().defaultBlockState(), 3);
        }
    ...
    private void placeSection(WorldGenLevel level, RandomSource random, BlockPos pos, int baseHeight, int iteration, int loop, double multiplier, boolean bottom) {
        ...
        for(int j = 0; j < baseHeight; j++) {
            ...
            else level.setBlock(location, ModBlocks.DARK_STONE.get().defaultBlockState(), 3);
        }
    }
    ...
    private void stretchToFloor(WorldGenLevel level, BlockPos pos, int i, int loop, boolean bottom) {
        ...
        if(bottom) {
            while(!level.getBlockState(blockPos).is(ModBlocks.DARK_STONE.get()) && !level.getBlockState(blockPos).is(Blocks.DEEPSLATE)) {
                level.setBlock(blockPos, ModBlocks.DARK_STONE.get().defaultBlockState(), 3);
                blockPos = blockPos.below();
            }
            return;
        }

        while(!level.getBlockState(blockPos).is(Blocks.SCULK) && !level.getBlockState(blockPos).is(ModBlocks.DARK_STONE.get()) && !level.getBlockState(blockPos).is(Blocks.DEEPSLATE)) {
            level.setBlock(blockPos, ModBlocks.DARK_STONE.get().defaultBlockState(), 3);
